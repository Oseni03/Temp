...
- **Redis Cache Keys:** The caching currently relies on the _domain_ level (`verify:domain:${domain}`) to prevent massive cache fragmentation across catch-all domains, though it returns the original email in the JSON response payload.
