            ...
            <header className="flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                    <p className="text-sm text-muted-foreground font-light">
                        Welcome back, <span className="text-foreground font-medium">{session.user.name.split(' ')[0]}</span>.
                    </p>
                    <div className="h-1 w-1 rounded-full bg-border" />
                    <p className="text-xs text-muted-foreground font-light italic">Viewing activity for the last 30 days.</p>
                </div>
            </header>
                ...
                <CardHeader className="p-8 border-b border-border/50 flex flex-row items-center justify-between group cursor-pointer">
                    <div className="space-y-1">
                        ...
                        <CardDescription className="text-sm font-light">
                            Live stream of verification requests across your infrastructure.
                        </CardDescription>
                    </div>
                    <Link href="/dashboard/logs" className="p-3 rounded-2xl bg-muted/50 border border-border/50 group-hover:bg-primary/10 group-hover:border-primary/20 transition-all">
                    ...
                    <div className="">
                        {recentActivity.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-24 text-center">
                                ...
                                <p className="text-sm text-muted-foreground font-light">No verification activity recorded yet.</p>
                                <Link href="/docs" className="mt-4 text-sm font-bold text-primary hover:underline">View API setup guide</Link>
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-border/50 text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60">
                                            <th className="h-14 px-8 text-left font-bold">Timestamp</th>
                                            <th className="h-14 px-8 text-left font-bold">API Key Alias</th>
                                            <th className="h-14 px-8 text-right font-bold">Response Status</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-border/50">
                                        {recentActivity.map((log) => (
                                            <tr key={log.id} className="group hover:bg-muted/[0.03] transition-colors">
