...
import { z } from "zod";
import SurveyResponseEmail from "@/components/emails/survey-response";
...
export async function submitSurvey(data: SurveyData) {
    ...
    const { challenge, otherChallenge, currentSolution, userEmail, userName } =
        result.data;
    ...
    try {
        if (
            ...
            return {
                success: true,
                message: "Survey submitted successfully (Simulated)",
            };
        }
